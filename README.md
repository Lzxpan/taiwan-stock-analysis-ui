...
修改原因：使用者回報刷新即時監控時 Gradio 顯示 `Connection errored out`。
修改前內容：即時監控在 `auto` / `official` 模式下，每 30 秒刷新會同時重跑官方日資料分析，6 檔股票可能耗時 19 到 40 秒，導致前端連線逾時。
修改後功能：版號升為 `V01.003`；即時報價仍使用 TWSE MIS / auto 即時來源，但監控警示所需的觀察區間改用本機快速分析，刷新時間降到約 1 秒內，避免 Gradio callback 逾時。
